...
import ROOT
from ROOT import TLine, TGraph, TCanvas, gPad, TFile
...
#####################################################################################################################
################### Start threshold scan for calibrated pixels (Moritz' falling edge detection) #####################
#####################################################################################################################
glib = a._hw
mpa = []
conf = []
...
glib.getNode("Configuration").getNode("num_MPA").write(0x6)
...
for thr in range(0,255):
    ...
    if thr%1 == 0:
        ...
        thrArray.append(thr)
        # Apply threshold
        config1.modifyperiphery('THDAC',[thr]*6)
        config1.upload()
        config1.write()
...
c2 = TCanvas("graph2","graph2",700,900)
c2.cd()    

for i,graph in enumerate(graphsCounter):
    if i == 0:
        graph.Draw("APL") 
...
for pxl in pxlVsThrCounter:
    ...
    pxl = [hits if hits < 20000 else 0 for hits in pxl] # Exclude extremely high noise as that is not plausible. Glib error?
    ...
    ## Find peak
    pxlDiff = [thr - max(pxl) for thr in pxl]
    thrArrayPeak.append(thrArray[pxlDiff.index(0)])

print "Falling edges at %s" %thrArrayFallingEdge

##########################################################################################
#################### Plot calibrated pixels (Nash's edge detection) ######################
##########################################################################################

print "Testing Calibration (NASH)"
...
c3.Print('plots/Scurve_Calibration'+options.string+'_post.root', 'root')
c3.Print('plots/Scurve_Calibration'+options.string+'_post.pdf', 'pdf')
c3.Print('plots/Scurve_Calibration'+options.string+'_post.png', 'png')
